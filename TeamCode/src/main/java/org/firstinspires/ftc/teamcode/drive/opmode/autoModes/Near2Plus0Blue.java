...
@Config
public class Near2Plus0Blue extends LinearOpMode {
    VisionPortal.Builder vPortalBuilder;
    VisionPortal vPortal;
    ElapsedTime timer2;
    ElapsedTime timer;
    final Pose2d startPose = new Pose2d(12,66, Math.toRadians(90));
            ...
            while (opModeInInit()) {
                ...
                if (itemSector == 0) {
                    driveToLine = drive.trajectoryBuilder(startPose)
                            .lineToLinearHeading(new Pose2d(16, isBlue * 54, Math.toRadians((-isBlue * 90) + ((itemSector - 1) * -14.4))))
                            .build();
                ...
                if (itemSector == 1) {
                    driveToBoard = drive.trajectorySequenceBuilder(driveToLine.end())
                            ...
                            .strafeLeft(-2) // strafes in front of appropriate AprilTag
                            .build();
                } else {
                    driveToBoard = drive.trajectorySequenceBuilder(driveToLine.end())
                            .splineToLinearHeading(new Pose2d(48, 60 * isBlue, Math.toRadians(180)), Math.toRadians(0)) // go to center of the tape
                            .strafeTo(new Vector2d(48, (37+(itemSector-1)*-9)* isBlue)) // strafes in front of appropriate AprilTag
                            .build();
                }
        ...
        switch(currentState) {
            ...
            case SCORE_YELLOW:
                if (previousState != currentState) {
                    ...
                    if (timer2.time() > 0.625) {
                        arm.rest();
                    }
                    if (arm.atAngle() & timer2.time() > 0.625) {
